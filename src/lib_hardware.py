    ...
    try:
        ...
        for partition in logical_disk.associators("Win32_LogicalDiskToPartition"):
            ...
            for physical_disk in partition.associators("Win32_DiskDriveToDiskPartition"):
                ...
                serial = physical_disk.SerialNumber.strip() if physical_disk.SerialNumber else "UNKNOWN_SERIAL"
                model = physical_disk.Model
                logging.info(f"Resolved {drive_letter} -> Model: {model}, Serial: {serial}")
                return serial
...
def scan_drives():
    ...
    try:
        ...
        for physical_disk in c.Win32_DiskDrive():
            ...
            is_usb = "USB" in physical_disk.InterfaceType.upper() if physical_disk.InterfaceType else False
            is_removable = physical_disk.MediaType and "REMOVABLE" in physical_disk.MediaType.upper()
            
            # Skip if neither (likely internal fixed disk)
            if not (is_usb or is_removable):
                continue
                
            serial = physical_disk.SerialNumber.strip() if physical_disk.SerialNumber else "UNKNOWN"
            model = physical_disk.Model
