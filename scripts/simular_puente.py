...
def run_simulation(test_name, total_sd_gb, free_int_gb, free_ext_gb):
    ...
    def mock_disk_usage(path):
        if path == external:
            return (free_ext_gb * 1024**3 * 2, free_ext_gb * 1024**3, free_ext_gb * 1024**3)
        if path == internal:
            return (free_int_gb * 1024**3 * 2, free_int_gb * 1024**3, free_int_gb * 1024**3)
    ...
    with patch("os.walk", side_effect=mock_walk), \
         ...
         patch("os.makedirs"), patch("shutil.rmtree"), \
         patch("shutil.move"), patch("os.remove"):
...
if __name__ == "__main__":
    ...
    run_simulation("Sufficient Space (Full Persist)", total_sd_gb=20, free_int_gb=50, free_ext_gb=100)
    ...
    run_simulation("Limited Internal Space (Chunked bridging)", total_sd_gb=20, free_int_gb=10, free_ext_gb=100)
    ...
    run_simulation("Critically Low Internal Space", total_sd_gb=20, free_int_gb=2.5, free_ext_gb=100)
    
    # Case 4: Not enough External Space
    run_simulation("Insufficient External Space", total_sd_gb=20, free_int_gb=50, free_ext_gb=10)
